...
loc = ("https://github.com/CIDARLAB/neural-optimizer/transfer_learning/dataset/DAFD_transfer_learning_regime_1.xlsx")
...
X_train, X_test, Y_train, Y_test = model_selection.train_test_split(X1, Y11, test_size=validation_size) #Regime 1 Output 1
...
TLmodel.add(Dense(units = 32, input_dim=8, activation='relu', name='dense_1', trainable=False))
...
### load 3rd layer weights and keep unchanged to avoid over-fitting
TLmodel.add(Dense(units = 16, activation='relu', name='dense3', trainable=False))
#TLmodel.add(Dropout(0.4))
### update 4th layer weights to fit the data
TLmodel.add(Dense(8, activation='relu', name='new_dense4'))
#TLmodel.add(Dropout(0.4))
### update last layer weights to fit the data
TLmodel.add(Dense(units = 1, name='new_dense5'))#, kernel_regularizer=regularizers.l2(0.001)))

#-----------------------------------------------------------------------------
#   Load the Pre-Trained Nueral Network Model
#-----------------------------------------------------------------------------
#Load saved weights
TLmodel.load_weights('Y11_weights.h5', by_name=True)
...
result = TLmodel.fit(X_train, Y_train, validation_data=(X_test, Y_test), batch_size = 1, epochs = 600, callbacks=[earlystopping])
...
y_pred_total = TLmodel.predict(X1)
...
print("\n")
print("Mean absolute error (MAE) for total data-set:      %f" % sklearn.metrics.mean_absolute_error(Y11,y_pred_total))
print("Mean squared error (MSE) for total data-set:       %f" % sklearn.metrics.mean_squared_error(Y11,y_pred_total))
print("Root mean squared error (RMSE) for total data-set: %f" % math.sqrt(sklearn.metrics.mean_squared_error(Y11,y_pred_total)))
print("R square (R^2) for total data-set:                 %f" % sklearn.metrics.r2_score(Y11,y_pred_total))